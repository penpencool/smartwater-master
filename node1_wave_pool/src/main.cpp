...
// ==========================================
// 1. PIN DEFINITIONS (Node 1 - Wave Pool)
// ==========================================
...
void setup() {
    ...
    Serial.println("==========================================");
    Serial.println("🌊 Node 1 (Wave Pool Float Sensor) READY!");
    Serial.println("==========================================");
}
...
void loop() {
    ...
    payload.nodeId = NODE_WAVE_POOL; // 1
    ...
    if (result == ESP_OK) {
        blinkStatusLed(1, 60);
        Serial.printf("📡 [Node 1] Sent: WaterLow=%s, Bat=%.2fV, MsgId=%u (SUCCESS)\n",
                      waterLow ? "YES (Need Fill)" : "NO (Full/OK)", batV, msgCount);
    } else {
        Serial.println("❌ [Node 1] ESP-NOW Send Failed!");
    }
